...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Create new user
  app.post("/api/game/user", async (req, res) => {
    try {
  ...
  app.post("/api/game/tap", async (req, res) => {
    try {
      ...
      const coinGain = taps * (user.tapProfit || 1);
      const newEnergy = Math.max(0, (user.energy || 5000) - taps);
      
      const updatedUser = await storage.updateUser(user.id, {
        dubaiCoin: (user.dubaiCoin || 0) + coinGain,
        energy: newEnergy
      });
  ...
  // Get market items (businesses)
  app.get("/api/game/market", async (req, res) => {
    try {
      const businesses = await storage.getAllBusinesses();
      res.json({ success: true, data: businesses });
    } catch (error) {
      res.status(500).json({ success: false, message: "Error fetching market" });
    }
  });
  ...
  // Use promo code
  app.post("/api/game/use-promo", async (req, res) => {
    try {
  ...
  app.post("/api/game/claim-offline", async (req, res) => {
    try {
      ...
      const actualHours = Math.min(hoursDiff, maxHours);
      
      const offlineEarnings = actualHours * (user.hourlyIncome || 0);
      
      const updatedUser = await storage.updateUser(user.id, {
        dubaiCoin: (user.dubaiCoin || 0) + offlineEarnings,
        lastActive: now
      });
      ...
      res.status(500).json({ success: false, message: "Error claiming offline earnings" });
    }
  });
