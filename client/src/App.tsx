...
const AppContent: React.FC = () => {
  const [activeTab, setActiveTab] = useState<string>('Imperiya');
  const [data, setData] = useState<GameData | null>(null);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [isInputModalOpen, setIsInputModalOpen] = useState<boolean>(false);
  const [inputModalConfig, setInputModalConfig] = useState<InputModalConfig | null>(null);
  const [paymentModalConfig, setPaymentModalConfig] = useState<PaymentModalConfig>({ show: false, item: null });
  const [selectedGame, setSelectedGame] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();
  const [hasShownLoadNotification, setHasShownLoadNotification] = useState<boolean>(false);
  ...
  const setUserState = useCallback((updater: ((user: GameUser) => GameUser) | Partial<GameUser>) => {
    setData((prev) => {
  ...
  const openPaymentModal = useCallback((item: PaymentModalConfig['item']) => {
